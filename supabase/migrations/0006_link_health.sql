-- 0006 — F34 link health. Additive synced columns + LWW RPC carry. Idempotent.
alter table bookmarks add column if not exists link_status text;
alter table bookmarks add column if not exists link_checked_at bigint;
alter table bookmarks add column if not exists link_redirect_url text;
